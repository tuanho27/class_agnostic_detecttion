...
class PolarCustomDataset(Dataset):
	...
	def __init__(self,
		...
		if num_samples is not None:
			...
			print("self.img_infos:", self.img_infos)
		...
		self.heatmap_transform = HeatmapTransform(size_divisor=self.size_divisor)
		self.segmap_transform = SegmapTransform(size_divisor=self.size_divisor)
		self.bbox_transform = BboxTransform()
		self.mask_transform = MaskTransform()
		self.seg_transform = SegMapTransform(size_divisor=self.size_divisor)
...
@DATASETS.register_module
class Coco_Seg_Dataset(PolarCustomDataset):
	...
	def prepare_train_img(self, idx):
		...
		if self.with_seg:
			gt_seg = mmcv.imread(
				osp.join(self.seg_prefix,
						 img_info['filename'].replace('jpg', 'png')),
				flag='unchanged')
			gt_seg = self.seg_transform(gt_seg.squeeze(), img_scale, flip, keep_ratio=self.resize_keep_ratio)
			gt_seg = mmcv.imrescale(
				gt_seg, self.seg_scale_factor, interpolation='nearest')
			gt_seg = gt_seg[None, ...]
		if self.proposals is not None:
		...
		if self.with_fg_mask:
			fg_mask = self.mask_transform(ann['masks'], pad_shape, scale_factor, flip)
			fg_mask = fg_mask.sum(axis=0).clip(0,1)
		...
		if self.with_seg:
			data['gt_semantic_seg'] = DC(to_tensor(gt_seg), stack=True)
		if self.with_fg_mask:
			data['gt_fg_mask'] = DC(to_tensor(fg_mask), stack=True)
