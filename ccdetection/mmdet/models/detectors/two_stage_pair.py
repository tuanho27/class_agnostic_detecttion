...
class TwoStagePairDetector(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def forward_pair_train(self,img,
        ...
        if self.with_rpn:
            ...
            rpn_loss_inputs_img0 = rpn_outs_img0 + (gt_bboxes[0], gt_labels[0],img_meta[0],
                                          self.train_cfg.rpn)
            ...
            proposal_inputs_img0 = rpn_outs_img0 + (img_meta[0], proposal_cfg, rpn_losses_img0['labels_list']) 
            # proposal_list_img0 = self.rpn_head.get_proposals_w_label(*proposal_inputs_img0)
            proposal_list_img0 = self.rpn_head.get_bboxes(*proposal_inputs_img0)


            ## for set image 1
            rpn_outs_img1 = self.rpn_head(x1)
            rpn_loss_inputs_img1 = rpn_outs_img1 + (gt_bboxes[1],gt_labels[1], img_meta[1],
                                          self.train_cfg.rpn)
            ...
            proposal_cfg = self.train_cfg.get('rpn_proposal',self.test_cfg.rpn)
            proposal_inputs_img1 = rpn_outs_img1 + (img_meta[1], proposal_cfg, rpn_losses_img1['labels_list'])
            # proposal_list_img1 = self.rpn_head.get_proposals_w_label(*proposal_inputs_img1)
